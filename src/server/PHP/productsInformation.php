  ...
  <body>
	...
	<main>
		<?php 
			...
			foreach ($rows as $row) {
				echo	'<tr><td>' . $row['pID'] . '</td><td>' . $row['pName'] . '</td><td>' . $row['description'] . '</td><td>' . $row['price'] . '</td><td>' . $row['category'] . '</td><td><a href="UserDetails.php?filter=' . $row['pID'] . '">Select Product</a></td></tr>';
			}
			echo '</table>';
		?>
	</main>
